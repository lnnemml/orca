...
- **`orbitalList.ts`** (pure, node-tested): `orbitalRows` marks exactly one **HOMO** (highest
  ...
  HOMO. Occupied = occupancy > 0.5 (measured MOs are 2.0/0.0, not a knife-edge). `homoIndex` is the
  seam the HOMO/LUMO marking and the F2 "HOMO+LUMO" button share.
- **`orbitalPalette.ts`** (pure, node-tested — F2): the ± phase colour PAIRS for simultaneous MOs.
  `ORBITAL_PHASE_PAIRS` — **pair 0 IS the viewer's blue/red default** (so a lone orbital is unchanged),
  pairs 1–3 distinguishable (live-tunable). `MAX_ORBITALS = 4`. `assignPairs(selected)` maps each MO
...
- **`OrbitalPanel.tsx`**: the picker is now **multi-select (cap 4)** — each active row shows its ±
  ...
  fetches **one cube per selected MO** via `Promise.allSettled(read_orbital_cube …)` (backend
  unchanged, one call per MO, each cube lazily generated by `orca_plot` and cached in the job dir keyed
  by MO+grid, read once capped at 32 MB — a **disk artifact, never in the DB**). **Honest-or-absent:**
  a MO whose cube is null/errors is **dropped** from the drawn set with a recorded reason (struck
  through in the legend), never a blank surface.
- **Isovalue is a DISPLAY choice** (like the FWHM and the mode amplitude): a slider, default **0.05**
  ...
  number, `orca-plot.md`).
- **State ownership (ADR-011):** the selected orbital(s), the shared isovalue and visibility are app
  state in `OrbitalPanel`; the viewer is handed the cube(s) + isovalue and draws. `MoleculeViewer` has
  an **`orbitalCube` / `orbitalCubes` / `orbitalIsoValue`** path: the molecule is built once from the
  first cube's atoms (model effect), and the **2N ± isosurfaces** (a pair per orbital) are drawn by a
  **dedicated single-owner effect** that parses each cube into a `VolumeData` **once** (cached by text
  in a `Map`, pruned to the live set) and, on an isovalue change, `removeShape`s exactly the full 2N
  set and re-adds it — no re-parse, and the scene-editor overlay effect is guarded to leave those
  shapes alone. **Single-orbital byte-identical:** `orbitalCube` (string) is the alias — it normalizes
  to a 1-element list with the blue/red default at opacity 0.85. Details of the array/normalization/
  opacity-by-N: [visualization.md](visualization.md#orbitals--densities-unit-315--done-f2--simultaneous-mos-done).
  **Seam (F2):** `orbitalCubes` is held in `OrbitalPanel` **state with a stable identity** (set once
