...
- [manual-keywords.md](modules/manual-keywords.md) — `src/manual/keywords.json` (`schema_version: 2`): the keyword→section map for the Monaco hover (4.4). Seeded from the manual's structured pool by a Rust `#[ignore]` generator, curated on top. **Block-options are qualified names** — key `(block, option)`, `block` derived by union of a text signal (literal `%block`, priority) and a structural proxy that agree **98.5 %**, `owner_source` records which, `null` is a value. Normalized (one `sections` array + int refs: 3173→317, ~0.56 MB); stable section key `(file, breadcrumb, title, nth)`; coverage post-condition 46/46 or the generator fails; `targets[]` reflects genuine multi-doc, not a guess
...
*Page count: 61. Last structural update: 2026-08-01 (+orca/input-syntax.md — measured ORCA block nesting, no-`end` directives, and Monaco word boundaries for the hover qualifier (unit 4.4 Part A); earlier +modules/manual-keywords.md — the seeded keyword→section map (unit 4.4 Part B): broad structured-pool home seeding, stable `(file, breadcrumb, title, nth)` key, 46/46 app-coverage post-condition, `targets[]` for the measured 14.2 % ambiguous tail).*
