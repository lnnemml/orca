...
use crate::commands::export_group::{
    build_manifest, slugify, CopyMode, GroupMeta, GroupNode, JobRow, ManifestV1, ResultRow,
};
...
fn export_group_conn(
    ...
    // Copy per job. A draft/never-run job (job_dir NULL) is in the manifest but copies
    // nothing and gets NO directory (no empty-dir clobber).
    ...
    for mjob in &manifest.jobs {
        ...
        for name in &mjob.files.included {
            ...
            // Best-effort per file: a vanished source file is skipped, not fatal — the
            // manifest still records it as intended-included.
...
mod tests {
    ...
    fn export_projects_group_to_ordered_dirs_and_manifest() {
        ...
        assert_eq!(m.source.group_name, "HCN reduction");
